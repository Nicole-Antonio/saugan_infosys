	...
	if(in_array($file_ext, $allowed_ext))
	{
		...
		foreach($data as $row)
		{
			if($count >0)
			{
				...
				$query = "INSERT INTO tests (`stud_id`,`subject`,`score`,`items`,`test_no`) VALUES ('$stud_id','$subject','$score','$items','$test_no')";
		...
		$_SESSION['massage'] = "Imported succesfully";
...
if(isset($_POST['export_excel_btn']))
{
    ...
    $fileName = "tests-sheet";

    $tests = "SELECT * FROM tests";
    $query_run = mysqli_query($con, $tests);
    ...
    if(mysqli_num_rows($query_run) > 0)
    {
        ...
        foreach($query_run as $data)
        {
            ...
            $sheet->setCellValue('E'.$rowCount, $data['test_no']);
            $rowCount++;
        }
